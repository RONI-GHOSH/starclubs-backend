...
$addMoneySql = "
    SELECT SUM(amount) AS AddMoneyTotal 
    FROM payment_proof 
    WHERE status = 'Approved' AND DATE(transaction_date) = CURRENT_DATE
";

$addMoneyQry = mysqli_query($conn, $addMoneySql);
$addMoneyRow = mysqli_fetch_assoc($addMoneyQry);
$amnum = $addMoneyRow['AddMoneyTotal'] ?: '0';

$pendingWithdrawSql = "
    SELECT SUM(transaction_amount) AS TotalPendingWithdraw 
    FROM wallet_transaction 
    WHERE withdrawl_status = 'pending' AND  transaction_type = 'WithdrawAmt' AND DATE(transaction_update_date) = CURRENT_DATE
";

// Execute the query
$pendingWithdrawQry = mysqli_query($conn, $pendingWithdrawSql);

// Fetch the result
$pendingWithdrawRow = mysqli_fetch_assoc($pendingWithdrawQry);

// Get the total amount, defaulting to 0 if null
$holdnum = $pendingWithdrawRow['TotalPendingWithdraw'] ?: '0';

$WithdrawSql = "
    SELECT SUM(transaction_amount) AS TotalPendingWithdraw 
    FROM wallet_transaction 
    WHERE withdrawl_status = 'approved' AND  transaction_type = 'WithdrawAmt' AND DATE(transaction_update_date) = CURRENT_DATE
";

// Execute the query
$WithdrawQry = mysqli_query($conn, $WithdrawSql);

// Fetch the result
$WithdrawRow = mysqli_fetch_assoc($WithdrawQry);

// Get the total amount, defaulting to 0 if null
$withnum = $WithdrawRow['TotalPendingWithdraw'] ?: '0';
