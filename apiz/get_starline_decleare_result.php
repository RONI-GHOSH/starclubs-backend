...
// print_r($_GET);
 ...
 $msql = "SELECT * FROM starlinemarketbat st LEFT JOIN member m ON st.member_id = m.member_id WHERE date(trim(st.betting_date)) = date(trim('$bid_date')) AND st.active_status='Active' AND (st.bet_num='$number' OR st.bet_num='$sm' OR st.bet_num='$nsm') AND st.statLineBatTime='$marketid'";
