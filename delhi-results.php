...
if (isset($_POST['delete-alloted'])) {
	...
	$winning_number_second = $lotterynumber[0];
	$winning_number_third = $lotterynumber[1];
	...
	$get = "UPDATE winningbetting_detail SET winning_number_first='', winning_number_second='', winning_number_third='', winning_number_fouth='' WHERE opening_date=date('$betdate') AND market_Id ='$market_id'";
	$gql = mysqli_query($conn, $get);

	if (isset($gql)) {
		echo '<script>alert("Number Deletion Successfull!");window.location = "delhi-results.php";</script>';
	} else {
		echo '<script>alert("Number Deletion Failed");window.location = "delhi-results.php";</script>';
	}
}
	...
	if (!mysqli_query($conn, $sql2)) {
		...
		if ($refer_type = 'Loss Betting') {
			...
			$lossbetmsql = "SELECT bt.*, m.*, mk.*, IFNULL(mr.referrer_id, '') AS referrer_id FROM betting bt 
				...
				AND b_market_id = '$market_id' AND betting_status = 'Active' AND (
												...
												<div class="row">
													...
													<div class="form-group col-md-3">
														...
														<div class="form-group">
															<input type="text" maxlength="2" required class="form-control" placeholder="Number" name="number" id="number">
														</div>
													</div>
													...
													<div class="form-group col-md-3">
														<label>&nbsp;</label>
														<button type="submit" class="btn btn-primary btn-block" name="update">Share Commission</button>
													</div>
													...
													<div class="form-group col-md-3">
														<label>&nbsp;</label>
														<button type="button" onclick="delhiGetResult(this);" class="btn btn-warning btn-block">Winner List</button>
													</div>

													<div class="form-group col-md-3">
														<label>&nbsp;</label>
														<button type="submit" class="btn btn-danger btn-block" name="delete-alloted">Delete Result</button>
													</div>
											...
											<tbody>
												...
												?>
														<tr>
															...
															<td><?php echo $row['winning_number_second'] . '-'; ?></td>
															<td><?php echo $row['winning_number_second'] . $row['winning_number_third']; ?></td>
															<td><?php echo  '-' . $row['winning_number_third']; ?></td>
	...
	<script>
		$(document).ready(function() {
			...
			function populateDeclaredResults() {
				...
				var gameName = $('#market_id').val();
				// alert(gameName);
				// AJAX request
				$.ajax({
					url: 'apiz/delhi_delete_results.php',
					type: 'POST',
					data: {
						betdate: betdate,
						gameName: gameName,
			...
			$('#betdate, #marketid').change(function() {
