...
if (isset($_POST['update-allot'])) {
    ...
    if (mysqli_num_rows($getqry) > 0) {
        ...
        if (!mysqli_query($conn, $usql)) {
            echo "<script>alert('Number Update Failed');window.location = 'color-decleare-result.php';</script>";
        } 
        else {
            ...
            if (!mysqli_query($conn, $dsql)) {
                ...
                if ($refer_type = 'Loss Betting') {
                    ...
                    game_name='Color' AND bet_num !='$number'";
                    ...
                    echo '<script>alert("Number Update Successfully");window.location = "color-decleare-result.php";</script>';
                } 
                elseif ($refer_type = 'Flat') {
                    $dateObject = new DateTime($betdate);
                    // Format the date as desired (y-m-d)
                    $formattedDate = $dateObject->format('y-m-d');
                    $lossbetmsql = "select * from colormarketbat WHERE statLineBatTime='$market_id' AND betting_date ='$formattedDate' AND game_name='Color'";
                    $lossbetquery = mysqli_query($conn, $lossbetmsql);
                    // Fetch and process each row
                    while ($row = mysqli_fetch_assoc($lossbetquery)) {
                        $member_id = $row['member_id'];
                                            ...
                                            <form class="theme-form mega-form" method="post">
                                                ...
                                                <div class="row">
                                                    <div class="form-group col-md-3">
                                                        ...
                                                        <div class="date-picker">
                                                            ...
                                                            <div class="input-group">
                                                                ...
                                                                <input class="form-control digits" type="hidden" value="Color" name="game_name" id="game_name">
                                                    ...
                                                    <div class="form-group col-md-2">
                                                        <label for="market_close">Color</label>
                                                        <select class="form-control" name="number" id="number">
                                                            <option value="">--Color Name--</option>
                                                            <?php

                                                            $getsql = "SELECT * FROM color WHERE status = '1'  ";
                                                            $getqry = mysqli_query($conn, $getsql);
                                                            $i = 1;
                                                            if (mysqli_num_rows($getqry) > 0) {
                                                                while ($row = mysqli_fetch_assoc($getqry)) {
