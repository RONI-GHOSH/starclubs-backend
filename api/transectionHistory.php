...
$date_to = $_POST['date_to'];
      ...
      $sql = "SELECT *  FROM betting bet INNER JOIN market mar  ON bet.b_market_id = mar.market_id  left JOIN game gam  ON bet.b_game_cid = gam.game_id INNER JOIN member mem  ON bet.b_member_id = mem .member_id  WHERE bet.betting_date BETWEEN Date('$date_from') AND Date('$date_to') AND bet.b_member_id ='$member_id' AND bet.betting_status = 'Active' ORDER BY bet.betting_id DESC";
...
if (mysqli_num_rows($result)) {
	...
	while ($rows = mysqli_fetch_assoc($result))
	{
	    ...
	    if($b_game_cid=="0")
	    {
	       ...
	       $bat_number="Bat Number ".$rows['betting_number'].' '.$rows['betting_number_second'];
