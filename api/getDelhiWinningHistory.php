...
$date_to = $_POST['date_to'];
    ...
    $sql = "SELECT wt.*, mem0.member_name AS sendername FROM wallet_transaction wt INNER JOIN member mem0 ON wt.member_id = mem0.member_id inner join market mar on mar.market_id=wt.market_id WHERE DAte(transaction_update_date) BETWEEN Date('$date_from') AND Date('$date_to') AND wt.member_id ='$member_id'  AND transaction_type='WinningBat' AND mar.market_type='Delhi'  ORDER BY w_transaction_id DESC";
...
if (mysqli_num_rows($result)) {
	...
	while ($rows = mysqli_fetch_assoc($result))
	{
	    ...
	    else if($transaction_type=="WinningBat")
	    {
	        ...
	        $newtransaction_type="Add";
	        $bat_number="Bet Number ".$rows['bat_number'];
	    }
